...
<html>
    <body>
        ...
        <form action="${pageContext.request.contextPath}/laptops" method="post" enctype="multipart/form-data">
            ...
            New Arrival : <input type="radio" name="newArrival" value="true"> TRUE <br/><input type="radio" name="newArrival" value="false"> FALSE<br/>
            <input type="submit" name="submit" value="Submit">
            <a href="${pageContext.request.contextPath}/statistics">
                <button type="button">Thống kê giá</button>
            </a>
        </form>
        <table border="1">
            <tr>
            ...
            <c:forEach items="${laptops}" var="l">
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${l.isFeatured}">Yes</c:when>
                            <c:otherwise>No</c:otherwise>
                        </c:choose>
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${l.isNewArrival}">Yes</c:when>
                            <c:otherwise>No</c:otherwise>
                        </c:choose>
                    </td>
                    <td>${l.description}</td>
